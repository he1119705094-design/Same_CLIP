...
def save_feature_map(tensor, name, img_name, idx=0, max_channels=4):
    save_dir = "feature_maps_fake_50%"
    os.makedirs(save_dir, exist_ok=True)
    # 去掉图片名和节点名中的特殊字符
    safe_img_name = re.sub(r'[\\/*?:"<>|]', "_", img_name)
    safe_name = re.sub(r'[\\/*?:"<>|]', "_", name)
    if tensor.dim() == 4:  # [B, C, H, W]
        feat = tensor[idx].detach().cpu()
        for i in range(min(max_channels, feat.shape[0])):
            plt.imshow(feat[i], cmap='viridis')
            plt.axis('off')
            plt.savefig(os.path.join(save_dir, f'{safe_img_name}_{safe_name}_ch{i}.png'), bbox_inches='tight')
            plt.close()
    elif tensor.dim() == 2:  # [B, F]
        feat = tensor[idx].detach().cpu().numpy()
        plt.plot(feat)
        plt.title(f"{safe_img_name}_{safe_name}")
        plt.savefig(os.path.join(save_dir, f'{safe_img_name}_{safe_name}_vector.png'))
        plt.close()
...
class ResNet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        save_feature_map(x, "afterconv1", self.current_img_name)
        x = self.layer1(x)
        save_feature_map(x, "afterlayer1", self.current_img_name)
        x = self.layer2(x)
        save_feature_map(x, "afterlayer2", self.current_img_name)
        x = self.layer3(x)
        save_feature_map(x, "afterlayer3", self.current_img_name)
        x = self.layer4(x)
        save_feature_map(x, "afterlayer4", self.current_img_name)
        return x
    ...
    def forward(self, x):
        # layer1之前
        save_feature_map(x, "origal", self.current_img_name)
